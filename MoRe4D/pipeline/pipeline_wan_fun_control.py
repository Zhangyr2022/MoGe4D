...
class WanFunControlPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        if control_camera_video is not None:
            ...
            # 把 depth_image repeat 成和 control_video_latents 一样的 shape
            if depth_image is not None:
                depth_image = depth_image.repeat(1, 1, video_length, 1, 1)
                depth_image = depth_image.to(dtype=torch.float32)
                depth_latents = self.prepare_control_latents(
                    None,
                    depth_image,
                    batch_size,
                    height,
        ...
        if clip_image is not None:
            ...
            print("DEBUG: x dtype/device:", clip_image.dtype, clip_image.device)
            # for n, p in self.clip_image_encoder.named_parameters():
            #     print("DEBUG: param", n, " dtype/device:", p.dtype, p.device)
            self.clip_image_encoder = self.clip_image_encoder
            clip_context = self.clip_image_encoder([clip_image[:, None, :, :]]).to(device, weight_dtype)
        else:
        ...
        if do_classifier_free_guidance:
            first_frame = first_frame if first_frame is None else torch.cat([first_frame] * 2)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if control_camera_video is not None:
                    ...
                    if depth_image is not None:
                        depth_latents_input = (
                            torch.cat([depth_latents] * 2) if do_classifier_free_guidance else depth_latents
                        ).to(device, weight_dtype)
                ...
                if depth_image is not None:
                    control_latents_input =  torch.cat([control_latents_input, depth_latents_input], dim = 1)
                clip_context_input = (
                ...
                with torch.cuda.amp.autocast(dtype=weight_dtype), torch.cuda.device(device=device):
                    if first_frame is None:
                        ...
                        noise_pred = self.transformer(
                            ...
                            full_ref=full_ref,
                            clip_fea=clip_context_input,
                            first_frame=first_frame[:,:,0].to(torch.bfloat16).to(device)
                        )
        ...
        elif output_type == "no_normalize":
            video = self.decode_latents_no_normalize(latents)
