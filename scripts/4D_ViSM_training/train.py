        ...
        def collate_fn(examples):
            ...
            for example in examples:
                if args.fix_sample_size is not None:
                    ...
                    # print("closest_size", closest_size, "resize_size", resize_size)
                    transform = transforms.Compose([
                        transforms.Resize(resize_size, interpolation=transforms.InterpolationMode.BILINEAR),  # Image.BICUBIC
                        transforms.CenterCrop(closest_size),
                    ])
                    norm = transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5], inplace=True)
                new_examples["pixel_values"].append(norm(transform(pixel_values)))
                new_examples["text"].append(example["text"])
                new_examples["projected_images"].append(norm(transform(example["projected_images"].permute((0,3,1,2)).contiguous()/255.)))
                new_examples["mask"].append(transform(example["mask"].permute((0,3,1,2)).contiguous()).mean(dim=1, keepdim=True))
                ...
                if args.train_mode != "normal":
                    mask = new_examples["mask"]
                    
                    # mask = get_random_mask(new_examples["pixel_values"][-1].size())
                    mask_pixel_values = new_examples["projected_images"][-1] * (1 - mask[-1]) 
                    ...
                    # new_examples["mask"].append(mask.permute(0, 3, 1, 2).contiguous())
