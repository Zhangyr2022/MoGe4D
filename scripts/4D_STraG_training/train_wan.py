...
def log_validation(vae, text_encoder, tokenizer, clip_image_encoder, transformer3d, args, config, accelerator, weight_dtype, global_step):
    try:
        ...
        transformer3d_val = WanTransformer4DModel.from_pretrained(
...
def parse_args():
    ...
    parser.add_argument(
        "--freeze_wan", action="store_true", help="Whether to freeze the weights of WAN."
    )
    parser.add_argument(
        "--use_omnimae_guidance", action="store_true", help="Whether to use omnimae feature as guidance."
    )
    parser.add_argument(
        "--normalize_track_z", action="store_true", help="Whether to normalize the z axis of the track."
    )
    parser.add_argument(
        "--max_sample_dataset", type=int, default=None, help="Max sample the dataset to this size."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--skip_large_depth", action="store_true", help="Whether to skip the data with large depth value."
    )
    parser.add_argument(
        "--original_clip", action="store_true", help="Whether to use the original clip."
    )
    parser.add_argument(
        "--revision",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help=("The dir of vae ckpt. "),
    )
    parser.add_argument(
        "--all_mask",
        action="store_true",
        help="Whether to use full mask for inpainting.",
    )
    parser.add_argument(
        "--train_data_meta",
        type=str,
...
def main():
    ...
    transformer3d = WanTransformer4DModel.from_pretrained(
        os.path.join(args.pretrained_model_name_or_path, config['transformer_additional_kwargs'].get('transformer_subpath', 'transformer')),
        transformer_additional_kwargs=OmegaConf.to_container(config['transformer_additional_kwargs'])
    ).to(weight_dtype)
    if transformer3d.patch_embedding:
        print({k: v.shape for k, v in transformer3d.patch_embedding.state_dict().items()})
        old_conv = transformer3d.patch_embedding
        ...
        if old_w.shape[1] == 48:
            ...
            transformer3d.patch_embedding = new_conv

            print(f"Replaced patch_embedding: old in_channels=48 -> new in_channels=64, out_channels={out_c}")
        # old_weight = transformer3d.patch_embedding.state_dict()["weight"]  # shape [out_channels, in_channels, depth, height, width]
        # if old_weight.shape[1] == 48:
        #     new_weight = torch.zeros((old_weight.shape[0], 64, old_weight.shape[2], old_weight.shape[3], old_weight.shape[4]))
        #     new_weight[:, :48, :, :, :] = old_weight
        #     patch_size = transformer3d.patch_size
        #     original_bias = transformer3d.patch_embedding.state_dict()["bias"].clone()
        #     transformer3d.patch_embedding = nn.Conv3d(
        #         64, 5120, kernel_size=patch_size, stride=patch_size)
        #     transformer3d.patch_embedding.state_dict()["weight"] = new_weight
        #     transformer3d.patch_embedding.state_dict()["bias"] = nn.Parameter(original_bias)
        #     print(f"Resized patch_embedding.proj.weight from {old_weight.shape} to {new_weight.shape}")
    ...
    if args.vae_ckpt_dir is not None:
        encoder_prompt = VAEEncoderadaptor()
        decoder_prompt = VAEDecoderadaptor()
        print(f"From checkpoint: {args.vae_ckpt_dir}")
        encoder_state_dict = torch.load(os.path.join(args.vae_ckpt_dir, "encoder_prompt", "pytorch_model.bin"))
        decoder_state_dict = torch.load(os.path.join(args.vae_ckpt_dir, "decoder_prompt", "pytorch_model.bin"))
        encoder_prompt.load_state_dict(encoder_state_dict)
        decoder_prompt.load_state_dict(decoder_state_dict)
        
        vae_model_path = os.path.join(args.vae_ckpt_dir, "vae", "pytorch_model.bin")
        if os.path.exists(vae_model_path):
            vae_state_dict = torch.load(vae_model_path)
            vae.load_state_dict(vae_state_dict)
    
    vae.eval()
    encoder_prompt.eval()
    decoder_prompt.eval()
    encoder_prompt.to(accelerator.device, dtype=weight_dtype)
    decoder_prompt.to(accelerator.device, dtype=weight_dtype)
    encoder_prompt.requires_grad_(False)
    decoder_prompt.requires_grad_(False)
    ...
    for name, param in transformer3d.named_parameters():
        for trainable_module_name in args.trainable_modules + args.trainable_modules_low_learning_rate:
            if trainable_module_name in name:
                if ("omnimae_extractor" not in name):   
    ...
    if args.use_ema:
        ...
        ema_transformer3d = WanTransformer4DModel.from_pretrained(
            os.path.join(args.pretrained_model_name_or_path, config['transformer_additional_kwargs'].get('transformer_subpath', 'transformer')),
            transformer_additional_kwargs=OmegaConf.to_container(config['transformer_additional_kwargs']),
        ).to(weight_dtype)

        ema_transformer3d = EMAModel(ema_transformer3d.parameters(), model_cls=WanTransformer4DModel, model_config=ema_transformer3d.config)
        ...
        if fsdp_stage != 0:
            ...
            def load_model_hook(models, input_dir):
                if args.use_ema:
                    ...
                    _, ema_kwargs = WanTransformer4DModel.load_config(ema_path, return_unused_kwargs=True)
                    load_model = WanTransformer4DModel.from_pretrained(
                        input_dir, subfolder="transformer_ema",
                        transformer_additional_kwargs=OmegaConf.to_container(config['transformer_additional_kwargs'])
                    )
                    load_model = EMAModel(load_model.parameters(), model_cls=WanTransformer4DModel, model_config=load_model.config)
    ...
    in_already = []
    for name, param in transformer3d.named_parameters():
        # if (not args.freeze_wan) or (args.freeze_wan and ("feature_adapter" in name or "spatial_guidance" in name)):
        high_lr_flag = False
        ...
        for trainable_module_name in args.trainable_modules:
            if trainable_module_name in name:
                ...
                if accelerator.is_main_process:
                    ...
                    # if "spatial_guidance_self" in name:
                    #     print(f"Set {name} to lr : {args.learning_rate * 0.1}")
                break
        if high_lr_flag:
            continue
    ...
    # print("Trainable",[param.shape for param in trainable_params_optim[0]['params']])
    if args.use_came:
    ...
    train_dataset = STraGDataset(
        args.train_data_meta, args.train_data_dir,max_sample_dataset = args.max_sample_dataset,normalize_track_z = args.normalize_track_z
    )
    ...
    if args.enable_bucket:
        ...
        def collate_fn(examples):
            ...
            for example in examples:
                # To 0~1
                pixel_values = example["pixel_values"].permute(0, 3, 1, 2).contiguous()
                pixel_values = pixel_values

                control_pixel_values = example["control_pixel_values"].contiguous()
                depth_pixel_values = example["flow_first_frame"].unsqueeze(0).permute(0, 3, 1, 2).contiguous()
                ...
                depth_pixel_values = depth_pixel_values[:,2,:,:].unsqueeze(1).repeat(1,3,1,1)
                depth_pixel_values = transform_no_normalize(depth_pixel_values)
                depth_pixel_values = torch.clamp(depth_pixel_values, min=0.0, max=10000.0)
                depth_pixel_values[depth_pixel_values != depth_pixel_values] = 1.0
                depth_pixel_values[depth_pixel_values == float('inf')] = 1.0
                depth_pixel_values[depth_pixel_values == float('-inf')] = 1.0
                depth_pixel_values[depth_pixel_values < 1e-5] = 1.0
                ...
                depth_pixel_values = depth_pixel_values.repeat(49, 1, 1, 1)
                # print(torch.min(depth_pixel_values), torch.max(depth_pixel_values))
                new_examples["pixel_values"].append(transform_no_normalize(pixel_values))
                new_examples["control_pixel_values"].append(transform_no_normalize(control_pixel_values))
                new_examples["depth_pixel_values"].append(depth_pixel_values)

                # print(new_examples["control_pixel_values"][-1].shape)
                if args.train_mode == "control_camera_ref":
                ...
                if args.train_mode != "control":
                    ...
                    ref_pixel_values = new_examples["control_pixel_values"][-1][clip_index].unsqueeze(0)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            try:
                ...
                with accelerator.accumulate(transformer3d):
                    ...
                    with torch.no_grad():
                        def _batch_encode_vae_flow(pixel_values):
                            pixel_values = rearrange(pixel_values, "b f c h w -> b c f h w")
                            bs = args.vae_mini_batch
                            new_pixel_values = []
                            for i in range(0, pixel_values.shape[0], bs):
                                pixel_values_bs = pixel_values[i : i + bs]
                                pixel_values_bs = encoder_prompt(pixel_values_bs) * 2 - 1
                        ...
                        if vae_stream_1 is not None:
                            ...
                            with torch.cuda.stream(vae_stream_1):
                                latents = _batch_encode_vae_flow(pixel_values)
                        else:
                            latents = _batch_encode_vae_flow(pixel_values)

                        if args.train_mode != "control_camera_ref":
                            control_latents = _batch_encode_vae(control_pixel_values)
                            # print("range of depth:", torch.min(depth_pixel_values), torch.max(depth_pixel_values), torch.mean(depth_pixel_values))
                            depth_latents = _batch_encode_vae(depth_pixel_values)
                            # Make control latents to zero
                            for bs_index in range(control_latents.size()[0]):
                                if rng is None:
                                    zero_init_control_latents_conv_in = np.random.choice([0, 1], p = [0.98, 0.02])
                                else:
                                    zero_init_control_latents_conv_in = rng.choice([0, 1], p = [0.98, 0.02])
                        ...
                        if args.train_mode != "control":
                            ...
                            for bs_index in range(ref_latents.size()[0]):
                                # if rng is None:
                                #     zero_init_ref_latents_conv_in = np.random.choice([0, 1], p = [0.00, 1.0])
                                # else:
                                #     zero_init_ref_latents_conv_in = rng.choice([0, 1], p = [0.00, 1.0])
                                # print(ref_latents_conv_in.shape,clip_idx[bs_index],latents.size())
                                ...
                                # print((ref_latents_conv_in !=0).sum())
                                if args.add_full_ref_image_in_self_attention:
                                    if rng is None:
                                        zero_init_full_ref_conv_in = np.random.choice([0, 1], p = [0.98, 0.02])
                                    else:
                                        zero_init_full_ref_conv_in = rng.choice([0, 1], p = [0.98, 0.02])
                            ...
                            if control_latents is None:
                                ...
                                control_latents = torch.cat([control_latents, ref_latents_conv_in, depth_latents], dim=1)
                            ...
                            for clip_pixel_value in clip_pixel_values:
                                ...
                                if rng is None:
                                    zero_init_clip_in = np.random.choice([True, False], p=[0.02, 0.98])
                                else:
                                    zero_init_clip_in = rng.choice([True, False], p=[0.02, 0.98])
                    ...
                    with torch.cuda.amp.autocast(dtype=weight_dtype), torch.cuda.device(device=accelerator.device):
                        # print(rgb_pixel_values[:,0].min(), rgb_pixel_values[:,0].max())
                        noise_pred = transformer3d(
                            ...
                            first_frame=((rgb_pixel_values[:,0]+1)/2.0)
                        )
                    ...
                    if (global_step > 50 and avg_loss > 0.25):
                        print("Abnormal Loss is {}, skipping update".format(loss))
                        optimizer.zero_grad(set_to_none=True)
                        # Sync the model weights across processes (if we are using distributed training).
                        accelerator.wait_for_everyone()
                        if args.use_ema:
                            ema_transformer3d.sync_ema_buffers(transformer3d)
                        continue
                ...
                logs = {"step_loss": avg_loss.detach().item(), "lr": lr_scheduler.get_last_lr()[0]}
                if accelerator.is_main_process:
                    print("tmp")
                ...
                print(f"Error processing batch {step} in epoch {epoch}: {e}")
                import traceback
                traceback.print_exc()
                optimizer.zero_grad(set_to_none=True)
                torch.cuda.empty_cache()
